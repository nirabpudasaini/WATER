...
public class CountryAdapter extends BaseExpandableListAdapter {
    ...
    public CountryAdapter(Context context) {
        mContext = context;
        countries = Utils.getCountries(mContext);
        Log.i("COUNTRY ADAPTER", "Country Size :" + String.valueOf(countries.size()));
        for (Country country : countries) {
            ...
            parentList.add(countryName);

            List<String> options = new ArrayList<String>();
            options.add("Maps");
            options.add("Summary");
            options.add("City Level Details");
